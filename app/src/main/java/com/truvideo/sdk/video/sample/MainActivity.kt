...
class MainActivity : ComponentActivity() {
    ...
    companion object {
        const val TAG: String = "TruvideoSdkVideo"
    }
    ...
    private fun Content() {
        ...
        fun editVideo(info: TruvideoSdkVideoInformation) {
            scope.launch {
                ...
                val path = try {
                    val resultPath =
                        "${context.getExternalFilesDir(Environment.DIRECTORY_DCIM)?.path ?: ""}/${System.currentTimeMillis()}.mp4"
                    videoEditScreen.open(
                        videoPath = info.path,
                        resultPath = resultPath
                    )
                } catch (exception: Exception) {
                    Log.d("TruvideoSdkVideo", "Error editing video: ${exception.message}")
                    null
                }
        ...
        fun deleteVideo(info: TruvideoSdkVideoInformation) {
            try {
                File(info.path).delete()
                files = files.toMutableList().apply { remove(info) }.toList()
            } catch (exception: Exception) {
        ...
        Box(modifier = Modifier.padding(16.dp)) {
            ...
            Column {
                                ...
                                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                                    files.forEach {
                                        FileListItem(
                                            ...
                                            onDeletePressed = { deleteVideo(it) }
                                        )
                                    }
                ...
                TruvideoButton(text = "Add files from assets", onPressed = { addFilesFromAssets() })

                Box(Modifier.height(8.dp))

                TruvideoButton(text = "Pick files", onPressed = { pickFiles() })
