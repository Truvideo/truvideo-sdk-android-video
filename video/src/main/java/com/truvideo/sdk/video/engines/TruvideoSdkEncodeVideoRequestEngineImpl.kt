...
internal class TruvideoSdkEncodeVideoRequestEngineImpl(
    ...
    override fun process(
        id: String, callback: TruvideoSdkVideoJoinCallback
    ) = encode(id, callback)
    ...
    private suspend fun encode(id: String) {
        authAdapter.validateAuthentication()
        ...
        suspendCoroutine { cont ->
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    // Fetch the request
                    val request = videoRequestRepository.getById(id)
                        ?: throw TruvideoSdkException("Video request not found")
