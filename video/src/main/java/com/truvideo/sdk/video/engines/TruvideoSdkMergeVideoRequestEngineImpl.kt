...
internal class TruvideoSdkMergeVideoRequestEngineImpl(
    ...
    private val generateMergeVideosCommandUseCase: GenerateMergeVideosCommandUseCase
    ...
    override suspend fun process(id: String) {
        ...
        suspendCoroutine { cont ->
            scope.launch {
                try {
                    // Fetch the request
                    val request = videoRequestRepository.getById(id) ?: throw TruvideoSdkVideoException("Video request not found")
                    ...
                    // Validate data
                    val data = request.mergeData ?: throw TruvideoSdkVideoException("Invalid request data")

                    // Update status
                    videoRequestRepository.tryChangeStatusToProcessing(id)

                    // Generate command
                    val command = generateMergeVideosCommandUseCase(
                        data.videoPaths,
                        data.resultPath,
                        data.receivedWidth,
                        data.receivedHeight,
                    ...
                    if (exception is TruvideoSdkVideoException) {
                        ...
                        cont.resumeWith(Result.failure(TruvideoSdkVideoException(exception.localizedMessage ?: "Unknown error")))
                    }
                }
            }
        }
        ...
        scope.launch {
            try {
                ...
                if (exception is TruvideoSdkVideoException) {
                    ...
                    callback.onError(TruvideoSdkVideoException(exception.localizedMessage ?: ""))
                }
            }
        }
    }
