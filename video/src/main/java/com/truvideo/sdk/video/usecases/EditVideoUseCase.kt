    ...
    suspend operator fun invoke(
        ...
        try {
            ...
            if (result.code != Config.RETURN_CODE_SUCCESS) {
                throw TruvideoSdkException("Error trimming video. FFmpeg code: ${result.code}")
            }

            val file = File(resultPath)
            if (!file.exists()) {
                throw TruvideoSdkException("Error trimming video. This might be caused due to an inconsistent timeframe. Timeframe might be greater than video length.")
            }
