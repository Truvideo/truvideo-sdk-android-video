    ...
    suspend operator fun invoke(
        ...
        try {
            ...
            if (startPosition != null || endPosition != null) {
                ...
                trimCommand =
                    "-ss ${timeToFFmpeg(effectiveStart)} -to ${timeToFFmpeg((effectiveEnd))}"
            }

            var rotationMetadata = ""
            if (rotation != null) {
                Log.d(TAG, "Rotation: ${rotation.name} ${rotation.value}")
                rotationMetadata = "-metadata:s:v:0 rotate=${rotation.value}"
            }
            val volumeCommand = "-af \"volume=$volume\""

            val command =
                "-y -i $videoPath $trimCommand -c:v copy -c:a aac $rotationMetadata $volumeCommand $resultPath"
            ...
            if (result.code != Config.RETURN_CODE_SUCCESS) {
                ...
                Log.d(
                    "TruvideoSdkVideo",
                    "Error editing the video. FFmpeg code: ${result.code}. Message: $message"
                )
                throw TruvideoSdkVideoException("Error editing the video")
            }

            val file = File(resultPath)
            if (!file.exists()) {
                throw TruvideoSdkVideoException("Error editing the video. This might be caused due to an inconsistent timeframe. Timeframe might be greater than video length.")
            }
