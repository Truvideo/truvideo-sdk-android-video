...
internal class OpenEditScreenUseCase(
    ...
    fun init(activity: ComponentActivity): TruvideoSdkVideoEditScreen {
        ...
        if (handler == null) {
            handler = TruvideoSdkVideoEditScreen(activity = activity)
...
class TruvideoSdkVideoEditScreen(
        ...
        activity.registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                val videoPath: String? =
                    intent?.getStringExtra(TruvideoSdkVideoEditActivity.OUTPUT_FILE_PATH_EXTRA)
    ...
    @Suppress("unused")
    fun open(
        videoPath: String, resultPath: String, callback: TruvideoSdkVideoCallback<String?>
    ) {
        scope.launch {
            try {
                val result = open(
