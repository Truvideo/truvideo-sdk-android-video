...
internal class OpenEditScreenUseCase(
    ...
    fun init(activity: ComponentActivity): TruvideoSdkVideoEditScreen {
        ...
        if (handler == null) {
            val permissionHandler = permissionUseCase.init(activity)
            handler = TruvideoSdkVideoEditScreen(
                activity = activity,
...
class TruvideoSdkVideoEditScreen(
    ...
    private val permissionHandler: TruvideoSdkPermissionHandler
) {
    ...
    private val startForResult =
        activity.registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
            if (result.resultCode == Activity.RESULT_OK) {
                ...
                // Handle the Intent
                val videoPath: String? = intent?.getStringExtra(TruvideoSdkVideoEditActivity.OUTPUT_FILE_PATH_EXTRA)
    ...
    suspend fun open(
        ...
        val readPermission = permissionHandler.askReadStoragePermission()
        if (!readPermission) {
            throw TruvideoSdkVideoException("Read storage permission denied")
        }

        val writePermission = permissionHandler.askWriteStoragePermission()
        if (!writePermission) {
            throw TruvideoSdkVideoException("Write storage permission denied")
        }
        ...
        val intent = Intent(activity, TruvideoSdkVideoEditActivity::class.java).apply {
            putExtra(TruvideoSdkVideoEditActivity.INPUT_FILE_PATH_EXTRA, videoPath)
            putExtra(TruvideoSdkVideoEditActivity.OUTPUT_FILE_PATH_EXTRA, resultPath)
        }
